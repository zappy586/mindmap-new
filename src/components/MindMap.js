...
const GraphWithCameraFocus = () => {
  ...
  const handleNodeClick1 = async (node) => {
    try {
      ...
      setBgcolor(lightenColor(nodeColorMappings[node.val], 0.75));
      ...
      const responseStream = await fetch(
        "http://localhost:8000/generate_node_description/",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div id="graph-container" style={{ height: "100vh", width: "100%" }}>
      ...
      <ForceGraph3D
        ...
        nodeThreeObject={(node) => {
          ...
          sprite.textHeight = 2;
          const spriteBackgroundColor = lightenColor(
            nodeColorMappings[node.val],
            0.75
          ); // Lighten the sprite color
